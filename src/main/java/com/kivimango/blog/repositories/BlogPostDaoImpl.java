...
public class BlogPostDaoImpl implements BlogPostRepository {
	...
	public Page<BlogPost> findAllByHidden(int pageNum, Boolean status) {
		...
		String query = "SELECT blogpost_id, SUBSTRING(content, 0, 100) as content, slug, title, edited, uploaded, hidden, string_agg("+Schema.TAGS_TABLE+".tag, ',') as tags FROM " + Schema.POSTS_TABLE
				+ " LEFT JOIN " + Schema.POST_TAGS_LOOKUP_TABLE + " ON "+Schema.POST_TAGS_LOOKUP_TABLE+".blog_post_id = " +Schema.POSTS_TABLE+".blogpost_id"
				+ " LEFT JOIN " + Schema.TAGS_TABLE+" ON "+Schema.POST_TAGS_LOOKUP_TABLE+".tags_id = " +Schema.TAGS_TABLE+".t_id"
				+ " WHERE hidden = false"
				+ " GROUP BY content, uploaded, blogpost_id, slug, title, edited, hidden ORDER BY uploaded DESC"
	...
	public List<BlogPost> findByTitleOrSlugOrContentOrTags(String query) {
		String like = "%" + query + "%";
		String sql = "SELECT blogpost_id, content, slug, title, edited, uploaded, hidden, string_agg("+Schema.TAGS_TABLE+".tag, ',') as tags FROM " + Schema.POSTS_TABLE
				+ " LEFT JOIN " + Schema.POST_TAGS_LOOKUP_TABLE + " ON "+Schema.POST_TAGS_LOOKUP_TABLE+".blog_post_id = " +Schema.POSTS_TABLE+".blogpost_id"
				+ " LEFT JOIN " + Schema.TAGS_TABLE+" ON "+Schema.POST_TAGS_LOOKUP_TABLE+".tags_id = " +Schema.TAGS_TABLE+".t_id"
	...
	public List<BlogPost> findFirst5ByOrderByUploadedDesc() {
		String query = "SELECT title, slug FROM " + Schema.POSTS_TABLE + " WHERE hidden = false ORDER BY uploaded DESC LIMIT 5;";
